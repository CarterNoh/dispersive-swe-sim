...
public:
	...
	static constexpr float CELLSIZE = 0.15f;	// cell size in one dimension (meters/cell)
	...
	static constexpr float TERRAIN_SCALE = 1.5f;    // scale of terrain features (meters)
	...
	static constexpr float SLOPE_LIMIT = 0.7f; 		// wave slope to start "breaking" (dissipating energy at high slope)
    ...
    static constexpr float WIND_SPEED   = 4.f;     // m/s, at 10 meters above surface

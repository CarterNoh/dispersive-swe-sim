...
public:
	...
	static constexpr int GRIDSIZE_X = 512; 				// number of cells in X dimension
	static constexpr int GRIDSIZE_Y = 512; 				// number of cells in X dimension
	static constexpr float DOMAIN_SIZE_X = 252.f; 		// size of total grid in X (meters)
	static constexpr float DOMAIN_SIZE_Y = 252.f; 		// size of total grid in Y (meters)
	static constexpr float CELLSIZE = DOMAIN_SIZE_X / GRIDSIZE_X;	// cell size in one dimension (meters/cell)
	static constexpr float TIMESTEP = 1.f / 60.f;		// simulation timestep (seconds)
	static constexpr float CFL_CONDITION = 0.25f;  		// max allowed CFL condition for stability
	static constexpr float MIN_WATER_HEIGHT = 0.001f; 	// minimum water height for stability (meters)
    static constexpr float SURFACE_TENSION = 0.001f;	// surface tension of water (Newtons)
    static constexpr float DENSITY = 999.f;				// water density (kg/m^3)

	// Terrain & Water Parameters
	static constexpr int TERRAIN_TYPE = 7; 		   		// 0 = flat, 1 = ramp, 2 = bumps, 3 = basins, 4 = beach, 5 = 1D hill, 6 = 2D hill, 7 = unreal landscape
    ...
    static constexpr float FILTER_SMALL = 0.f;      // cutoff for small wavelengths (meters) 
    static constexpr float FILTER_BIG   = 10000.f;  // cutoff for large wavelengths (meters)
    static constexpr float FILTER_WIDTH = 1.f;      // filter dropoff width (meters)
    static constexpr float FILTER_MIN   = 0.01f;    // minimum filter strength (dimensionless)
	static constexpr float DEPTH_CUTOFF = 4.f; 		// depth to start attenuating FFT waves (meters)

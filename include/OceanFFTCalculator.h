...
#define OCEAN_SCOPE_CYCLE_COUNTER(Name) \
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT(#Name), STAT_##Name, STATGROUP_Ocean)
...
template <class TValue>
TValue FOceanFFTCalculator::BilinearInterpolation(
	float  U,
	float  V,
	TValue Value00,
	TValue Value01,
	...
	TValue NewValue = (OneMinusfX * OneMinusfY) * Value00 + (OneMinusfX * fY) * Value01
		+ (fX * OneMinusfY) * Value10 + (fX * fY) * Value11;

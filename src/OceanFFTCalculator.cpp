...
void FOceanFFTCalculator::Initialize() {
	checkf(
		(BatchSize * BATCH_COUNT) == OceanData.GridSize,
		TEXT("GridSize should be evenly divisible by BatchCount."));
...
void FOceanFFTCalculator::Calculate(UWorld* World) {
	...
	if (CalculatedEngineTime >= World->TimeSeconds)
		return;
		...
		for (int x = StartX; x < StartX + BatchSize; x++) {
			for (int y = 0; y < OceanData.GridSize; y++) {
				for (int z = 0; z < OceanData.NumCascades; z++) {
					...
					int32 Index = (x - OceanData.HalfGridSize)
						+ (y - OceanData.HalfGridSize) * GPU_GRID_SIZE
						+ z * GPU_GRID_SIZE * GPU_GRID_SIZE;
					float Random1 =
						Random(Index, 0, 0, RandomCounterDeterministic) * 2 * PI;
					float Random2 =
						Random(Index, 0, 0, RandomCounterDeterministic) * 2 * PI;
					float Random3 =
						Random(Index, 0, 0, RandomCounterDeterministic) * 2 * PI;
					...
					FVector4 RandomMagnitudes =
						FVector4(Random1, Random1, Random2, Random2);
					FVector4 RandomPhases = FVector4(
						FMath::Sin(Random3),
						FMath::Cos(Random3),
						FMath::Sin(Random4),
						FMath::Cos(Random4));
...
void FOceanFFTCalculator::PopulateSpectrum(
	FVector4   OneMinusWindDirectionality,
	FVector4   ComplexAmplitudes,
	FIntVector ThreadId) {
	// // Retrive WaveVector from thread index.
	FIntPoint Position = FIntPoint(ThreadId.X, ThreadId.Y);
	FVector2D WaveVector = FVector2D(
		Position.X - OceanData.HalfGridSize, Position.Y - OceanData.HalfGridSize);
	WaveVector *= 2.0f * PI;
	...
	// And our Z component of thread index is the index of cascade, current compute
	// thread belongs to.
	...
	if (k > 0.0001f) {
		...
		WindFactorAbs.X *=
			WindFactor.X > 0 ? 1 : OneMinusWindDirectionality[ThreadId.Z];
		WindFactorAbs.Y *=
			WindFactor.Y > 0 ? 1 : OneMinusWindDirectionality[ThreadId.Z];
		...
		Result = FVector2D(
			FMath::Sqrt(Spectrum * WindFactor.X), FMath::Sqrt(Spectrum * WindFactor.Y));
		Result *= 1.0f / FMath::Sqrt(2.0f);
	}

	// Lastly, obtain complex amplitude/phases.
	FVector4 PositiveAndNegativeSpectrum =
		FVector4(Result.X, Result.X, Result.Y, Result.Y);
...
float FOceanFFTCalculator::Random(
	int32  Seed1,
	int32  Seed2,
	int32  Seed3,
...
void FOceanFFTCalculator::CalculateGridTimeStep(float AnimationTime) {
	...
	ParallelFor(BATCH_COUNT, [&](int32 BatchIndex) {
		...
		ispc::FOceanFFTCalculator_TimeStepRow(
			StartX, StartX + BatchSize, AnimationTime, (ispc::FOceanFFTData&)OceanData);
	});
}
...
void FOceanFFTCalculator::CalculateRowPasses() {
	ParallelFor(BATCH_COUNT, [&](int32 BatchIndex) {
		...
		for (int Y = StartY; Y < StartY + BatchSize; Y++) {
			for (int32 CascadeIndex = 0; CascadeIndex < OceanData.NumCascades;
				 CascadeIndex++) {
				CalculateRowPass(
					Y, CascadeIndex, PingPongArrayX, PingPongArrayY, PingPongArrayZ);
			}
		}
	});
}
...
void FOceanFFTCalculator::ShowDebugDisplacementPoints(
	UWorld*		   World,
	...
	if (CVarShowOceanDisplacement.GetValueOnGameThread()) {
		...
		FVector Location = FVector(
			FMath::RoundHalfFromZero(CharacterLocation.X * DebugGridCellSize)
				/ DebugGridCellSize,
			FMath::RoundHalfFromZero(CharacterLocation.Y * DebugGridCellSize)
				/ DebugGridCellSize,
			0);
		...
		for (int Y = 0; Y < DebugGridSize; Y++) {
			for (int X = 0; X < DebugGridSize; X++) {
				...
				DrawDebugPoint(
					World, GridPointLocation + Displacement, 5.f, Color, false, 0.f, 0);
...
FVector
FOceanFFTCalculator::GetCascadeValue(FVector PointLocation, int32 CascadeIndex) {
	float U = FMath::Frac(
		PointLocation.X / OceanData.PatchLength[CascadeIndex] / CentimetersPerMeter);
	float V = FMath::Frac(
		PointLocation.Y / OceanData.PatchLength[CascadeIndex] / CentimetersPerMeter);
	...
	return BilinearInterpolation(
		...
		V,
		FVector(
			OceanData.DisplacementGridX[Index00],
			OceanData.DisplacementGridY[Index00],
			OceanData.DisplacementGridZ[Index00]), // Value00
		FVector(
			OceanData.DisplacementGridX[Index01],
			OceanData.DisplacementGridY[Index01],
			OceanData.DisplacementGridZ[Index01]), // Value01
		FVector(
			OceanData.DisplacementGridX[Index10],
			OceanData.DisplacementGridY[Index10],
			OceanData.DisplacementGridZ[Index10]), // Value10
		FVector(
			OceanData.DisplacementGridX[Index11],
			OceanData.DisplacementGridY[Index11],
			OceanData.DisplacementGridZ[Index11]), // Value11
		OceanData.GridSize);
}
...
FIntVector4 FOceanFFTCalculator::GetBoundingArrayIndexesFromUV(
	float U,
	float V,
	int32 ArraySize,
	bool  bWrap) {
